...
jQuery(document).ready(function () {
    ...
    jQuery('.minus').click(function () {
        if (parseInt(jQuery('.nums').html(), 10) != 1) {
            jQuery('.nums').html(parseInt(jQuery('.nums').html(), 10) - 1);
            checkGoodsAmount();
        }
...
function chooseSku() {
    ...
    jQuery('html,body').animate({
        scrollTop: jQuery('.skuInfo').offset().top - 270
    }, 500);
...
function buySubmit() {
    ...
    //                }else{
    var goodsAmount = parseInt(jQuery('#goodsAmount').html(), 10); // 要购买商品数量
...
function addCart(obj) {
    ...
    var goodsAmount = parseInt(jQuery('#goodsAmount').html(), 10);
